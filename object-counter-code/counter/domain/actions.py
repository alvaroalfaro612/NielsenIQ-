...
class CountDetectedObjects:
    ...
    def get_predictions_list(self,image,threshold): #returns a list with the class names of the detected objects over the given threshold in the image
        predictions = self.__find_valid_predictions(image, threshold)
        return [prediction.class_name for prediction in predictions]
